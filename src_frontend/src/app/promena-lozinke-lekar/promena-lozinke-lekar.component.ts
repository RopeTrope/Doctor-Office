...
@Component({
  selector: 'app-promena-lozinke-lekar',
  templateUrl: './promena-lozinke-lekar.component.html',
  styleUrls: ['./promena-lozinke-lekar.component.css']
})
export class PromenaLozinkeLekarComponent implements OnInit{
  ...
  promeniLozinku(){
    ...
    if(br && spec && velika && mala && prvo){
      this.error = ""
      this.userService.promenaLozinkeLekar(this.korisnik,this.old_pass).subscribe((l:Lekar)=>{
        if(l){
          this.userService.azurirajSifruLekar(this.korisnik,this.new_pass).subscribe();
